...
unsigned __stdcall ThreadSend(void* param){
	...
	sprintf_s(g_Client[flag].buf, "%s: %s", g_Client[!flag].userName, temp);//添加一个用户名头
...
unsigned __stdcall ThreadAccept(void* param){
	...
	while (1){
		while (i < 2){
			...
			*/
			char sendBuf[20] = {'\0' };
			inet_ntop(AF_INET, (void*)& g_ClientAddr.sin_addr, sendBuf, 16);
			printf("Successfuuly got a connection from IP:%s ,Port: %d,UerName: %s\n",
				sendBuf, htons(g_ClientAddr.sin_port), g_Client[i].userName);
			memcpy(g_Client[i].IP, sendBuf, sizeof(g_Client[i].IP)); //记录客户端IP
...
int StartServer(){
	...
	//开始线程执行， ThreadAccept为线程入口，参数为NUL，
